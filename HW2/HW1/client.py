...
# Configurazione URL - Ora puntiamo tutto all'API Gateway (NGINX)
# NGINX smisterà le richieste in base al percorso (/users o /interests)
...
# --- 1. REGISTRAZIONE ---
def register_user():
    # Registra un nuovo utente (At-Most-Once) passando per NGINX
    ...
    try:
        # Nota: La richiesta va alla porta 8080, NGINX la gira al container user-manager:5000
...
# --- 2. CANCELLAZIONE ---
def delete_user():
    # Elimina utente passando per NGINX
...
# --- 3. AGGIUNGI INTERESSE (AGGIORNATO HW2) ---
def add_interest():
    # Aggiunge aeroporto e opzionalmente le soglie per gli allarmi
    ...
    # Input per le nuove funzionalità di Alert (HW2)
    high_val = None
    low_val = None
    
    want_thresholds = input("Vuoi impostare soglie di allerta per le notifiche? (s/n): ").lower()
    ...
    try:
        ...
        if res.status_code == 200: # 200 perché ora gestiamo anche l'update
...
def _select_user_airport(email):
    # Recupera lista aeroporti tramite NGINX
    try:
...
def main_menu():
    while True:
        print("\n" + "="*40)
        print("   SKY MONITOR CLIENT (Via NGINX Gateway)")
        print("="*40)
        print("1. Registra Utente")
        print("2. Elimina Utente")
        print("3. Gestione Monitoraggio & Allarmi (HW2)")
        print("4. Ultimo Volo")
        print("5. Statistiche")
        print("0. Esci")
...
if __name__ == "__main__":
    # Test rapido connessione Gateway
    try:
        # Proviamo a chiamare NGINX (che girerà la richiesta a user-manager)
        requests.get(f"{GATEWAY_URL}/users", timeout=1) 
    except:
        print("Attenzione: Impossibile contattare l'API Gateway su localhost:8080.")
        print("Assicurati che Docker sia attivo: 'docker compose up'")
        time.sleep(2)
