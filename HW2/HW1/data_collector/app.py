...
# --- CIRCUIT BREAKER ---
# Se falliscono 3 richieste consecutive, apre il circuito per 60 secondi
...
# Applichiamo il Circuit Breaker a questa funzione critica
@open_sky_breaker
...
def fetch_flights_for_airport(icao_code, direction='arrival', hours_ago=1):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"RATE LIMIT HIT for {icao_code}. Skipping.")
            return []
        else:
        ...
        print(f"⚠️ CIRCUIT BREAKER OPEN: OpenSky is unreachable or failing. Skipping {icao_code}.")
        return []
    except requests.exceptions.RequestException as e:
        print(f"Network Error: {e}")
        return []

# --- WORKER CICLICO CON KAFKA ---

def fetch_flight_data_worker():
    # Attesa iniziale per dare tempo a Kafka di avviarsi
    time.sleep(15)
    ...
    try:
        ...
        print(f"❌ Warning: Kafka not available ({e}). Alerts will not be sent.")
            ...
            for airport_code in airports:
                ...
                if kafka_producer:
                    try:
                        ...
                        print(f" -> Sent Kafka update for {airport_code} (Total: {total_flights_count})")
...
def add_interest():
    """Aggiunge o aggiorna un aeroporto di interesse con soglie opzionali."""
    data = request.json
    ...
    try:
        ...
        # Query che gestisce sia inserimento che aggiornamento (ON DUPLICATE KEY UPDATE)
        query = """
