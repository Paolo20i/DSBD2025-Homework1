...
def start_alert_system():
    print("Alert System starting... waiting for Kafka...")
    time.sleep(10)  # Attesa fisiologica per l'avvio di Kafka

    # Producer per inviare le notifiche
    producer = KafkaProducer(
        bootstrap_servers=KAFKA_BOOTSTRAP_SERVERS,
        value_serializer=lambda v: json.dumps(v).encode('utf-8')
    )

    # Consumer per leggere i dati dal Data Collector
    ...
    for message in consumer:
        ...
        total_flights = data.get('total') # Assumiamo che il Data Collector invii questo campo
        
        print(f"Received data for {airport_code}: {total_flights} flights")

        # Controllo nel DB se qualcuno è interessato a queste soglie
        try:
            conn = get_db_connection()
            cursor = conn.cursor(dictionary=True)
            
            # Seleziona utenti interessati a questo aeroporto con soglie impostate
            query = """
            ...
            for user in interested_users:
                ...
                # Logica di superamento soglia (come da specifica)
                ...
                if alert_type:
                    # Crea il messaggio per il Notifier
                    alert_payload = {
                        "email": email,
